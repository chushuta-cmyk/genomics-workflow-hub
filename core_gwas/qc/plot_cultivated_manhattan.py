...
#!/usr/bin/env python3
"""
Generate Manhattan plots for cultivated soybean GWAS results.
"""
...
def manhattan_qq(gwas_file, title="Trait", save_path="manhattan_qq.png"):
    """Generate Manhattan and QQ plots."""
    df = read_gwas_table(gwas_file)
    
    # Filter chromosomes 1-20 (and handle scaffold entries)
    ...
    ax1.set_title(f"Cultivated Soybean: {title} - Manhattan Plot", fontsize=12)
    ...
    ax2.set_title(f"Cultivated Soybean: {title} - QQ Plot", fontsize=12)
...
def main():
    """Main function to generate all Manhattan plots for cultivated data."""
    output_dir = "results/cultivated/reports"
    os.makedirs(output_dir, exist_ok=True)
    
    # Define traits and corresponding files (using extracted GWAS tables)
    traits = [
        {
            "name": "100SW",
            "file": "results/cultivated/gwas/gwas_100SW.tsv",
            "output": f"{output_dir}/cultivated_manhattan_100SW.png"
        },
        {
            "name": "Protein",
            "file": "results/cultivated/gwas/gwas_Protein.tsv",
            "output": f"{output_dir}/cultivated_manhattan_Protein.png"
        },
        {
            "name": "Oil",
            "file": "results/cultivated/gwas/gwas_Oil.tsv",
            "output": f"{output_dir}/cultivated_manhattan_Oil.png"
        },
        {
            "name": "log_ratio",
            "file": "results/cultivated/gwas/gwas_log_ratio.tsv",
            "output": f"{output_dir}/cultivated_manhattan_log_ratio.png"
        }
    ]
