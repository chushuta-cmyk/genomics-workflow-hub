...
# Configuration
GWAS_FILES = {
    'size': 'data/input/workflow/output/size_analysis.assoc.txt',
    'protein': 'data/input/workflow/output/protein_analysis.assoc.txt',
    'oil': 'data/input/workflow/output/oil_analysis.assoc.txt'
}
...
COLUMNS = ['chr', 'rs', 'ps', 'allele1', 'allele0', 'af', 'beta', 'se', 'n_miss']
...
def extract_locus_gwas(trait, locus_chr, locus_start, locus_end):
    ...
    awk_cmd = f"""awk -F'\\t' '
BEGIN {{OFS="\\t"}}
NR==1 {{print "chr", "rs", "ps", "allele1", "allele0", "af", "beta", "se", "n_miss"}}
$1=="{locus_chr}" && $3>={locus_start} && $3<={locus_end} {{print $1, $2, $3, $5, $6, $7, $8, $9, $4}}
' "{gwas_file}" """
...
def prepare_coloc_input(df, trait):
    ...
    # Ensure MAF is minor allele frequency (already is)
    # If MAF > 0.5, flip to minor allele (but GEMMA already reports af of allele1)
    # We'll assume af is for allele1
    
    # Select columns needed for coloc
    coloc_cols = ['snp', 'position', 'beta', 'varbeta', 'MAF', 'N']
